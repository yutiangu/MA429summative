...
train_bar = as.data.frame(coupon_bar[-test_indices_bar,]);test_bar = as.data.frame(coupon_bar[test_indices_bar,])
train_carry = as.data.frame(coupon_carry[-test_indices_carry,]);test_carry = as.data.frame(coupon_carry[test_indices_carry,])
train_coffee = as.data.frame(coupon_coffee[-test_indices_coffee,]);test_coffee = as.data.frame(coupon_coffee[test_indices_coffee,])
train_restaurant20 = as.data.frame(coupon_restaurant20[-test_indices_restaurant20,]);test_restaurant20 = as.data.frame(coupon_restaurant20[test_indices_restaurant20,])
train_restaurant2050 = as.data.frame(coupon_restaurant2050[-test_indices_restaurant2050,]);test_restaurant2050 = as.data.frame(coupon_restaurant2050[test_indices_restaurant2050,])

train = bind_rows(train_bar,train_carry,train_coffee,train_restaurant20,train_restaurant2050)
test = bind_rows(test_bar,test_carry,test_coffee,test_restaurant20,test_restaurant2050)

# imputation
train_mice = train[,-which(colnames(train)=="Y")]
test_mice = test[,-which(colnames(test)=="Y")]

# imputation on training set:
train_impute = mice(train_mice, maxit=0)
meth = train_impute$method;meth
predM = train_impute$predictorMatrix
predM[, c("destination","passanger", "weather", "temperature", "time", "coupon", "expiration",
          "Bar", "CoffeeHouse", "CarryAway", "RestaurantLessThan20","Restaurant20To50", "direction_same", "toCoupon")]=0
predM
train_imputed <- mice(train_mice, method = meth, predictorMatrix = predM, m = 5)
train_mice <- mice::complete(train_imputed)
sapply(train_mice, function(x) sum(is.na(x)))

# imputation on test set:
test_impute = mice(test_mice, maxit=0)
meth = test_impute$method
meth
predM = test_impute$predictorMatrix
predM[, c("destination","passanger", "weather", "temperature", "time", "coupon", "expiration",
          "Bar", "CoffeeHouse", "CarryAway", "RestaurantLessThan20","Restaurant20To50", "direction_same", "toCoupon")]=0
predM
test_imputed <- mice(test_mice, method = meth, predictorMatrix = predM, m = 5)
test_mice <- mice::complete(test_imputed)
sapply(test_mice, function(x) sum(is.na(x)))

# complete data:
train = cbind(train_mice,Y = train$Y)
test = cbind(test_mice,Y = test$Y)
data_complete = rbind(train,test)
sapply(data_complete, function(x) sum(is.na(x)))

coupon_bar <- filter(data_complete, data_complete$coupon == "Bar")
coupon_carry <- filter(data_complete, data_complete$coupon == "Carry out & Take away")
coupon_coffee <- filter(data_complete, data_complete$coupon == "Coffee House")
coupon_restaurant20 <- filter(data_complete, data_complete$coupon == "Restaurant(<20)")
coupon_restaurant2050 <- filter(data_complete, data_complete$coupon == "Restaurant(20-50)")
train_bar = as.data.frame(coupon_bar[-test_indices_bar,]);test_bar = as.data.frame(coupon_bar[test_indices_bar,])
train_carry = as.data.frame(coupon_carry[-test_indices_carry,]);test_carry = as.data.frame(coupon_carry[test_indices_carry,])
train_coffee = as.data.frame(coupon_coffee[-test_indices_coffee,]);test_coffee = as.data.frame(coupon_coffee[test_indices_coffee,])
train_restaurant20 = as.data.frame(coupon_restaurant20[-test_indices_restaurant20,]);test_restaurant20 = as.data.frame(coupon_restaurant20[test_indices_restaurant20,])
